            ...
            for para_produce_t1 in "${para_produce_t1s[@]}"; do
                for para_produce_t2 in "${para_produce_t2s[@]}"; do
                    ...
                    for ((experiment=1; experiment<=num_experiments; experiment++)); do
                        ...
                        save_name="randomwalk_speed=${randomwalk_speed}-alpha=${alpha}-gradient_speed=${gradient_speed}-para_produce_t1=${para_produce_t1}-para_produce_t2=${para_produce_t2}/${random_suffix}"
                        sbatch ./scripts/cpuN1C2_param_identification.sbatch "-c" "./examples/vacv-epithelial/dVGF_default_config.ini" "--save_name" $save_name "--randomwalk_speed" "$randomwalk_speed" "--alpha" "$alpha" "--gradient_speed" "$gradient_speed" "--para_produce_t1" "$para_produce_t1" "--para_produce_t2" "$para_produce_t2"
                    done
                done
            done
        done
